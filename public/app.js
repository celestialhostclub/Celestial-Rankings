...
const $=s=>document.querySelector(s);let period='month',request=0,allPlayers=[],searchActive=false,rankingState='loading';const date=d=>new Date(d+'T12:00:00').toLocaleDateString('pt-BR',{day:'2-digit',month:'short'}).replace('.','');const escape=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const initials=p=>escape(p.name.slice(0,2).toUpperCase());const rate=p=>`<span class="rate"><span class="bar"><i style="width:${p.rate}%"></i></span><strong>${p.rate}%</strong></span>`;
...
function renderRanking(players){players=players.slice(0,10);if(!players.length){$('#ranking-content').innerHTML='<div class="state">Ainda não há resultados neste período.<br>Selecione outro mês para acompanhar os jogadores.</div>';return;}$('#ranking-content').innerHTML=`<div class="ranking-wrap"><table><caption class="sr">Ranking de jogadores por confrontos vencidos</caption><thead><tr><th scope="col">Pos.</th><th scope="col">Jogador</th><th scope="col">Tag</th><th scope="col">Treinos<br>jogados</th><th scope="col">Confrontos<br>vencidos</th><th scope="col">Confrontos<br>perdidos</th><th scope="col">Mapas<br>vencidos</th><th scope="col">Mapas<br>perdidos</th><th scope="col">Taxa de<br>vitória</th></tr></thead><tbody>${players.map((p,i)=>`<tr class="rank-${i+1}"><td><span class="rank-number">${String(i+1).padStart(2,'0')}</span></td><td><span class="player">${playerPortrait()}${escape(p.name)}</span></td><td class="tag">${escape(p.tag)}</td><td>${p.trainings}</td><td class="win">${p.wins}</td><td class="loss">${p.losses}</td><td>${p.mapsWon}</td><td class="loss">${p.mapsLost}</td><td>${rate(p)}</td></tr>`).join('')}</tbody></table></div><div class="mobile-ranking">${players.map((p,i)=>`<article class="player-card rank-${i+1}"><div class="player-top"><span class="rank-number">${i+1}</span>${playerPortrait()}<div><strong>${escape(p.name)}</strong><span class="tag">${escape(p.tag)}</span></div>${rate(p)}</div><div class="player-card-stats"><div><small>Treinos jogados</small><strong>${p.trainings}</strong></div><div><small>Confrontos</small><strong>${p.wins} V <span class="loss">/ ${p.losses} D</span></strong></div><div><small>Mapas</small><strong>${p.mapsWon} V <span class="loss">/ ${p.mapsLost} D</span></strong></div></div></article>`).join('')}</div>`;}
async function load(){const id=++request;$('#ranking-content').setAttribute('aria-busy','true');$('#ranking-content').innerHTML='<div class="skeleton" role="status" aria-label="Carregando ranking"></div>';allPlayers=[];rankingState='loading';$('#search-submit').disabled=true;if(searchActive)renderPlayerSearch();try{const data=await getDashboard({period,month:$('#month').value});if(id!==request)return;allPlayers=data.players;rankingState='ready';$('#search-submit').disabled=false;renderRanking(allPlayers);if(searchActive)renderPlayerSearch();$('#ranking-content').setAttribute('aria-busy','false');enter(document.querySelectorAll('.ranking-wrap tbody tr,.player-card'),{distance:5,stagger:18,duration:260});$('#summary').innerHTML=[['Treinos realizados',data.summary.trainings,'no período selecionado','swords'],['Mapas disputados',data.summary.maps,'cada mapa, uma oportunidade','map'],['Jogadores ranqueados',data.summary.players,'na disputa pelo topo','users'],['Última atualização',data.summary.updated,'Dados demonstrativos','clock']].map(([label,value,sub,icon])=>`<article><span class="stat-icon" aria-hidden="true">${iconSvg(icon)}</span><span class="stat-label">${label}</span><strong>${value}</strong><small>${sub}</small></article>`).join('');enter(document.querySelectorAll('.summary article'),{distance:6,stagger:30,duration:300});}catch(e){if(id!==request)return;$('#ranking-content').setAttribute('aria-busy','false');$('#ranking-content').innerHTML='<div class="state" role="alert">Não foi possível carregar os resultados.<button id="retry">Tentar novamente</button></div>';$('#retry').onclick=load;rankingState='error';$('#search-submit').disabled=false;if(searchActive)renderPlayerSearch();}}
document.querySelectorAll('[data-period]').forEach(b=>b.onclick=()=>{period=b.dataset.period;document.querySelectorAll('[data-period]').forEach(x=>x.setAttribute('aria-pressed',x===b));$('#month').disabled=period==='all';load();});$('#month').onchange=load;
$('#training').innerHTML=sessions.map(s=>`<option value="${s.id}">${s.name} · ${date(s.date)} 2026</option>`).join('');
function renderMatches(){
 const session=sessions.find(s=>s.id===$('#training').value);
 ...
 $('#training-title').textContent=`${session.name} · ${date(session.date)} de 2026`;
 $('#match-count').textContent=`${session.matches.length} confrontos`;
 $('#matches').innerHTML=session.matches.length?session.matches.map((m,i)=>`<article class="match"><div class="match-meta"><span>Confronto ${String(i+1).padStart(2,'0')}</span><span class="status">Finalizado</span></div><div class="score"><button class="team-link ${m.mapsA<m.mapsB?'loser':''}" data-session="${session.id}" data-team="${m.a}">Time ${m.a+1}</button><strong><span class="${m.mapsA<m.mapsB?'loser':''}">${m.mapsA}</span><em>×</em><span class="${m.mapsB<m.mapsA?'loser':''}">${m.mapsB}</span></strong><button class="team-link ${m.mapsB<m.mapsA?'loser':''}" data-session="${session.id}" data-team="${m.b}">Time ${m.b+1}</button></div><div class="match-foot"><span>${m.mapsA+m.mapsB} mapas disputados</span><span class="${m.extended?'extended':''}">${m.extended?`Série estendida: ${m.mapsA} × ${m.mapsB}`:'Confronto encerrado'}</span></div><button class="roster-button" data-session="${session.id}" data-match="${m.id}" aria-label="Ver detalhes de Time ${m.a+1} contra Time ${m.b+1}">${iconSvg('users')} Ver times e desempenho</button></article>`).join(''):'<p class="state">Nenhum confronto registrado neste treino.</p>';
}
...
document.addEventListener('click',event=>{
 ...
 const selected=match ? session.teams.filter(t=>t.id===match.a||t.id===match.b) : session.teams.filter(t=>t.id===Number(trigger.dataset.team));
 if(!selected.length)return;
 $('#team-dialog-title').textContent=match?'Times do confronto':`${selected[0].name} · Detalhes`;
 $('#team-dialog-context').textContent=`${session.name} · ${date(session.date)} de 2026`;
...
function renderPlayerSearch(){
 ...
 if(!p){result.innerHTML='<p class="search-message">Nenhum resultado para essa tag neste período. Confira a tag ou escolha outro período. Os dados atuais são demonstrativos.</p>';return;}
